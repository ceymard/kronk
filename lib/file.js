...
exports.File = class File {
  ...
  render(meta = {}) {
    ...
    meta = deepmerge.all([
      {kronk: {output_filename: this.name + '.html'}},
      defaults,
      this.meta,
      meta
    ])
    ...
    if (rendered) {
      ...
      var pth = path.join(this.$conf.build, meta.kronk.output_filename)
  ...
  renderMd(contents, meta) {
    var md = new Markdown(meta.kronk.markdown_options)
    var md_contents = md.render(contents)
    var pug_template = this.meta.kronk.markdown_template
    var pug_block = this.meta.kronk.markdown_block
...
`, meta)
  ...
  renderPug(contents, meta) {
    return pug.compile(contents, deepmerge({
      filename: this.name,
      basedir: this.$conf.templates,
      pretty: _(meta, 'pug.pretty', false)
    }, meta.kronk.pug_options))
    (Object.assign({
      $file: this,
      $files: this.$files,
    }, meta))
